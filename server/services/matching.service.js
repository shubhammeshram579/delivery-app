 ...
 * matching.service.js
 *
 * Deterministic driver matching engine.
 * Score = Distance*40% + Rating*20% + AcceptanceRate*20% + Workload*20%
 *
 * Flow:
 *   Driver cancels/system assigns → assignBestDriver() → offer to top-scored driver
 *   → driver has 30s to respond (socket push + timeout)
 *   → accept: done | reject/timeout: recordRejectionAndRetry() tries next candidate
 *   → exhausted candidates: notifyAdminAssignmentFailed()
 */
