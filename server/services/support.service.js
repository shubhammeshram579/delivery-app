...
const addMessageToTicket = async (ticketId, senderId, senderType, message) => {
  ...
  // Handle notifications depending on who sent the message
  if (senderType === "admin") {
    await ticket.update({ status: "waiting_on_user" });
    
    // NOTIFICATION TO USER (Driver/Customer): Uses standard sendNotification
    await sendNotification(ticket.userId, {
      title: "Support team replied",
      body: message.slice(0, 100),
      type: "support",
    ...
    // NOTIFICATION TO SPECIFIC ASSIGNED ADMIN:
    // Note: Since this targets a single, specific admin instead of broadcasting to all, 
    // it uses standard sendNotification for the unique admin ID. 
    if (ticket.assignedAdminId) {
      await sendNotification(ticket.assignedAdminId, {
        title: `New message — Ticket #${ticket.ticketNumber}`,
        body: message.slice(0, 100),
