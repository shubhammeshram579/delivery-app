...
export default function AIPackagingAdvisor({ itemDescription }) {
  ...
  return (
    <div className="card mt-3 p-3 bg-blue-50 border border-blue-100 rounded-xl animate-in">
      ...
      {loading ? (
        <div className="card flex items-center gap-2 text-xs text-blue-500">
          <span className="w-3 h-3 border-2 border-blue-200 border-t-blue-500 rounded-full animate-spin" />
          Analyzing item...
        </div>
      ) : advice ? (
        <div className="text-xs text-blue-700 dark:text-gray-400 leading-relaxed whitespace-pre-line flex gap-2">
