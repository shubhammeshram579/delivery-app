...
import {
  LifeBuoy, ChevronRight, ArrowLeft, UserPlus, Send
} from 'lucide-react';
...
const PRIORITY_STYLE = {
  ...
  normal:   { bg: 'bg-yellow-100', text: 'text-yellow-700', dot: 'bg-yellow-500' },
...
const STATUS_LABEL = {
  ...
  waiting_on_user: 'Waiting on User', // Kept for missing information pauses
  resolved: 'Resolved', 
  closed: 'Closed',
};
...
export default function AdminSupportDashboard() {
  ...
  // ── Primitive state filters to stop the infinite re-render loop ──
  const [statusFilter, setStatusFilter] = useState('');
  const [priorityFilter, setPriorityFilter] = useState('');
  
  const { socket } = useSocket();

  // Memoize filters safely so loadTickets maintains a stable dependency signature
  ...
  useEffect(() => {
    ...
    const handler = (data) => {
      toast(`New ${data.priority} ticket: ${data.subject}`, {
        icon: data.priority === 'critical' ? '🔴' : data.priority === 'high' ? '🟠' : '🟡',
        duration: 6000,
      });
      loadTickets();
    };
  ...
  if (selectedId) {
    return <TicketDetail ticketId={selectedId} onBack={() => { setSelectedId(null); loadTickets(); }} />;
  }
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-400 mt-0.5">
                      {t.user?.name} ({t.userType}) · #{t.ticketNumber} · {t.createdAt ? formatDistanceToNow(new Date(t.createdAt), { addSuffix: true }) : ''}
                    </p>
                  </div>
...
function TicketDetail({ ticketId, onBack }) {
  ...
  useEffect(() => {
    ...
    const handler = (msg) => {
      ...
      setMessages((prev) => {
        const checkExists = prev.some((m) => m.id === msg.id || (m.tempId && m.tempId === msg.tempId));
  ...
  const send = async () => {
    ...
    setInput(''); // Optimistically clear fields immediately
    setSending(true);
    
    const tempId = `temp-${Date.now()}`;
    
    // Optimistic UI update to ensure lightning-fast display responses
    ...
    try {
      if (socket?.current?.connected) {
        socket.current.emit('support:message', { ticketId, message: currentText, senderType: 'admin', tempId });
      } else {
      ...
      // Remove the message if saving crashes down the road
    ...
    <div className="grid lg:grid-cols-3 gap-5">
      <div className="lg:col-span-2 card flex flex-col" style={{ height: '600px' }}>
        <div className="flex items-center gap-3 px-5 py-4 border-b border-gray-100">
          ...
          <span className={`px-2 py-0.5 rounded-full text-xs font-semibold ${p.bg} ${p.text}`}>{ticket.priority}</span>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-3 bg-gray-50">
          ...
          {messages.map((m, i) => (
            <div key={m.id || i} className={`flex ${m.senderType === 'admin' ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[75%] px-3.5 py-2.5 rounded-2xl text-sm ${
                m.senderType === 'admin' ? 'bg-primary-600 text-white rounded-tr-sm'
                : m.senderType === 'ai' ? 'bg-purple-50 text-purple-800 border border-purple-100 rounded-tl-sm'
                : 'bg-white border border-gray-100 text-gray-800 rounded-tl-sm'
              }`}>
                {m.senderType !== 'admin' && (
                  <p className="text-[10px] font-semibold opacity-60 mb-0.5 uppercase">{m.senderType}</p>
                )}
                {m.message}
              </div>
            </div>
        ...
        <div className="p-4 border-t border-gray-100 flex gap-2">
          ...
          />
          <button onClick={send} disabled={sending || !input.trim()} className="btn-primary px-4 disabled:opacity-50">
      ...
      <div className="space-y-4">
        <div className="card p-5">
          <h3 className="text-sm font-semibold text-gray-800 mb-3">Ticket Info</h3>
          <div className="space-y-3 text-sm">
            <div>
              <p className="text-xs text-gray-400">Customer/Driver</p>
              <p className="font-medium">{ticket.user?.name} ({ticket.userType})</p>
            ...
            <div>
              ...
              <p>{ticket.source === 'ai_escalated' ? '🤖 AI Escalated' : '👤 User Initiated'}</p>
            </div>
            <div>
          ...
          {!ticket.assignedAdmin && (
            <button onClick={assignToMe} className="btn-secondary w-full mt-4 text-sm flex items-center justify-center gap-2">
        ...
        <div className="card p-5">
          <h3 className="text-sm font-semibold text-gray-800 mb-3">Update Priority</h3>
            ...
            {['low', 'normal', 'high', 'critical'].map((pr) => (
              <button
                ...
                className={`text-xs py-2 rounded-lg font-medium capitalize border-2 transition-colors ${
                  ticket.priority === pr ? `${PRIORITY_STYLE[pr].bg} ${PRIORITY_STYLE[pr].text} border-current` : 'border-gray-200 text-gray-500'
                }`}
              >
                {pr}
              </button>
            ...
            {Object.entries(STATUS_LABEL).map(([k, v]) => (
              <button
                ...
                className={`w-full text-left text-xs py-2 px-3 rounded-lg transition-colors ${
                  ticket.status === k ? 'bg-primary-50 text-primary-700 font-medium' : 'hover:bg-gray-50 text-gray-600'
                }`}
              >
                {v}
              </button>
