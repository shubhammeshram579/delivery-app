...
import { MessageSquare, User, ChevronRight, Package } from 'lucide-react';
import { useRequireAuth } from '../../../components/shared/AuthGuard';
import { DashboardLayout } from '../../../components/shared/Layout';
      ...
      <div className="max-w-2xl">
        {loading ? (
          ...
          <EmptyState
            ...
            description="Accept an order to start chatting with customers."
          />
        ) : (
          <div className="card divide-y divide-gray-50">
            {conversations.map((conv) => {
              ...
              return (
                <button
                  key={conv.id}
                  onClick={() => router.push(`/driver/orders/${conv.id}`)}
                  className="w-full flex items-center gap-4 px-5 py-4 hover:bg-gray-50 transition-colors text-left"
                >
                  <div className="w-11 h-11 bg-orange-100 rounded-full flex items-center justify-center text-orange-700 font-bold text-lg flex-shrink-0">
                    {customer?.name?.[0] || '?'}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between gap-2 mb-0.5">
                      <p className="text-sm font-semibold text-gray-900 truncate">
                        {customer?.name || 'Customer'} — #{conv.orderNumber}
                      </p>
                      {lastMsg && (
