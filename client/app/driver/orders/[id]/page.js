...
export default function DriverOrderDetailPage() {
  ...
  const initMap = useCallback(() => {
    ...
    drawRoute(map, pickupLatLng, dropLatLng, routeRendererRef, "#0390fc");
    setMapReady(true);
  }, [order]);
  ...
  useEffect(() => {
    ...
    if (order.status === "accepted" && locationRef.current) {
      ...
      drawRoute(map, driverPos, pickupLatLng, routeRendererRef, "#f97316");
      const bounds = new window.google.maps.LatLngBounds();
      bounds.extend(driverPos);
      bounds.extend(pickupLatLng);
      map.fitBounds(bounds, { top: 60, right: 40, bottom: 40, left: 40 });
    ...
    if (order.status === "picked_up" || order.status === "in_transit") {
      drawRoute(map, pickupLatLng, dropLatLng, routeRendererRef, "#2563eb");
      const bounds = new window.google.maps.LatLngBounds();
      bounds.extend(pickupLatLng);
      bounds.extend(dropLatLng);
      map.fitBounds(bounds, { top: 60, right: 40, bottom: 40, left: 40 });
  ...
  return (
    <DashboardLayout role="driver" title={`Order #${order.orderNumber}`}>
      {/* Container with responsive ordering: controls on top for mobile, grid for desktop */}
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-4 sm:gap-5">
        
        {/* ── Map Column (Rendered first on Mobile for route view, or order reversed if needed) ── */}
        <div className="lg:col-span-3 space-y-3 order-1 lg:order-2">
          <div className="card overflow-hidden h-[350px] sm:h-[450px] lg:h-[520px]">
            {/* Map header */}
            <div className="flex flex-wrap items-center justify-between gap-2 px-3 sm:px-4 py-2.5 sm:py-3 border-b border-gray-100 dark:border-gray-800">
              <h3 className="text-xs sm:text-sm font-semibold text-gray-800 dark:text-gray-300">
                {mapLabel}
              </h3>
              <div className="flex flex-wrap gap-2 sm:gap-3 text-[11px] sm:text-xs text-gray-500">
                <span className="flex items-center gap-1">
                  <span className="w-2 h-2 rounded-full bg-green-500 inline-block" />{" "}
                  Pickup
                </span>
            ...
            >
              {!process.env.NEXT_PUBLIC_GOOGLE_MAPS_KEY && (
                <div className="h-full flex flex-col items-center justify-center bg-gray-50 dark:bg-gray-800/50 p-4 gap-3">
                  <MapPin className="h-8 w-8 sm:h-10 sm:w-10 text-gray-300" />
                  ...
                  <div className="text-center w-full max-w-xs">
                    ...
                    <div className="flex flex-col sm:flex-row gap-2">
                      ...
                      <button
                        ...
                        className="btn-primary text-xs py-2 px-3 w-full"
                      >
                        Open Drop
                      </button>
                    </div>
          ...
          <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
            <button
              ...
              className="btn-secondary flex-1 flex items-center justify-center gap-2 text-xs sm:text-sm py-2.5 sm:py-2"
            >
              <Navigation className="h-4 w-4" /> Navigate to Pickup
            </button>
            <button
              ...
              className="btn-primary flex-1 flex items-center justify-center gap-2 text-xs sm:text-sm py-2.5 sm:py-2"
            >
              <Navigation className="h-4 w-4" /> Navigate to Drop
            </button>
          </div>
        ...
        <div className="lg:col-span-2 space-y-4 order-2 lg:order-1">
          ...
          <div className="card p-4 sm:p-5">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-semibold text-sm sm:text-base text-gray-800 dark:text-gray-300">
                {isPassenger ? "Booking" : "Order Status"}
              </h3>
              <StatusBadge status={order.status} />
            </div>
            ...
            <div className="space-y-3 mb-4">
              ...
              <div className="flex gap-2.5 sm:gap-3 items-start">
                <MapPin className="h-4 w-4 text-green-500 mt-0.5 flex-shrink-0" />
                <div className="flex-1 min-w-0">
                  <p className="text-[11px] sm:text-xs text-gray-400">Pickup</p>
                  <p className="text-xs sm:text-sm text-gray-700 dark:text-gray-400 break-words line-clamp-2">
                    {order.pickupAddress}
                  </p>
                </div>
                <button
                  ...
                  className="w-8 h-8 bg-blue-50 dark:bg-blue-900/20 rounded-lg flex items-center justify-center flex-shrink-0"
                  title="Navigate to pickup"
                >
              ...
              <div className="flex gap-2.5 sm:gap-3 items-start">
                <MapPin className="h-4 w-4 text-red-500 mt-0.5 flex-shrink-0" />
                <div className="flex-1 min-w-0">
                  <p className="text-[11px] sm:text-xs text-gray-400">Drop</p>
                  <p className="text-xs sm:text-sm text-gray-700 dark:text-gray-400 break-words line-clamp-2">
                    {order.dropAddress}
                  </p>
                </div>
                <button
                  ...
                  className="w-8 h-8 bg-red-50 dark:bg-red-900/20 rounded-lg flex items-center justify-center flex-shrink-0"
                  title="Navigate to drop"
                >
                  <Navigation className="h-3.5 w-3.5 text-red-500 dark:text-red-400" />
                </button>
              </div>
            </div>

            {/* Stats strip */}
            <div className="grid grid-cols-3 gap-1 sm:gap-2 pt-3 border-t border-gray-100 dark:border-gray-800 text-center text-xs">
              <div>
                <p className="text-gray-400 text-[11px] sm:text-xs">Distance</p>
                <p className="font-semibold text-xs sm:text-sm text-gray-800 dark:text-gray-200">
                  {order.distance?.toFixed(1)} km
                </p>
              </div>
              <div>
                <p className="text-gray-400 text-[11px] sm:text-xs">ETA</p>
                <p className="font-semibold text-xs sm:text-sm text-gray-800 dark:text-gray-200">
                  {order.estimatedTime} min
                </p>
              </div>
              <div>
                <p className="text-gray-400 text-[11px] sm:text-xs">Earning</p>
          ...
          {/* Customer info */}
          <div className="card p-4 sm:p-5">
            <h3 className="font-semibold text-sm sm:text-base text-gray-800 dark:text-gray-300 mb-3">Customer</h3>
            <div className="flex items-center gap-3">
              <div className="w-9 h-9 sm:w-10 sm:h-10 bg-primary-100 rounded-full flex items-center justify-center text-primary-700 font-semibold text-sm flex-shrink-0">
                {order.customer?.name?.[0]}
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-xs sm:text-sm font-medium text-gray-900 dark:text-gray-300 truncate">
                  {order.customer?.name}
                </p>
                <a
                  href={`tel:${order.customer?.phone}`}
                  className="text-xs text-primary-600 dark:text-primary-400 hover:underline block truncate"
                >
                  {order.customer?.phone}
                </a>
              </div>
              <a
                href={`tel:${order.customer?.phone}`}
                className="w-8 h-8 sm:w-9 sm:h-9 bg-green-50 dark:bg-green-900/20 rounded-lg flex items-center justify-center flex-shrink-0"
              >
            ...
            >
              {isCash ? (
                <>
                  <Banknote className="h-3.5 w-3.5 flex-shrink-0" /> Cash on Delivery
                </>
              ) : (
                <>
                  <IndianRupee className="h-3.5 w-3.5 flex-shrink-0" /> Online Payment
                </>
              )}
              {isCash && cashDone && (
                <span className="ml-auto flex items-center gap-1 text-green-600 dark:text-green-400">
                  <CheckCircle className="h-3.5 w-3.5 flex-shrink-0" /> Collected
                </span>
              )}
            </div>
            ...
            {['accepted', 'picked_up'].includes(order.status) && (
              <button 
                ...
                className="mt-3 sm:mt-4 w-full bg-red-100 dark:bg-red-950/40 hover:bg-red-200 dark:hover:bg-red-900/50 text-red-600 dark:text-red-400 font-medium py-2 sm:py-2.5 rounded-xl transition-colors text-xs sm:text-sm"
              >
                Cancel This Delivery
              </button>
            )}
          ...
          <div className="space-y-3">
            ...
            {order.status === "accepted" && !pickupOtpVerified && (
              <div className="card p-3.5 sm:p-4 border-l-4 border-l-yellow-400 bg-yellow-50 dark:bg-yellow-950/20">
                <p className="text-xs sm:text-sm font-medium text-gray-800 dark:text-gray-200 mb-1 flex items-center gap-2">
                  <KeyRound className="h-4 w-4 text-yellow-500 flex-shrink-0" />
                  Verify Pickup OTP
                </p>
            ...
            {order.status === "accepted" && pickupOtpVerified && (
              <div className="flex items-center gap-2 px-3 py-2 bg-green-50 dark:bg-green-950/30 rounded-lg text-xs text-green-700 dark:text-green-400 font-medium">
                <CheckCircle className="h-3.5 w-3.5 flex-shrink-0" /> Pickup OTP verified
              </div>
            )}
            ...
            {order.status === "in_transit" &&
              !isPassenger &&
              !deliveryOtpVerified && (
                <div className="card p-3.5 sm:p-4 border-l-4 border-l-blue-400 bg-blue-50 dark:bg-blue-950/20">
                  <p className="text-xs sm:text-sm font-medium text-gray-800 dark:text-gray-200 mb-1 flex items-center gap-2">
            ...
            {order.status === "in_transit" &&
              ...
              deliveryOtpVerified && (
                <div className="flex items-center gap-2 px-3 py-2 bg-green-50 dark:bg-green-950/30 rounded-lg text-xs text-green-700 dark:text-green-400 font-medium">
                  <CheckCircle className="h-3.5 w-3.5 flex-shrink-0" /> Receiver OTP verified
                </div>
              )}

            {order.status === "in_transit" &&
              (isPassenger || deliveryOtpVerified) && (
                <div className="card p-3.5 sm:p-4 border-l-4 border-l-green-400 bg-green-50 dark:bg-green-950/20">
                  <p className="text-xs sm:text-sm font-medium text-gray-800 dark:text-gray-200 mb-1 flex items-center gap-2">
                  ...
                  <button
                    onClick={hadelShowProofModal}
                    className="w-full py-2.5 sm:py-3 text-xs sm:text-sm bg-green-600 hover:bg-green-700 text-white font-medium rounded-lg flex items-center justify-center gap-2 transition-colors"
                  >
                    <Camera className="h-4 w-4" />
                    Upload Proof & Mark Delivered
                  </button>
            ...
            {isCash &&
              ...
              ["in_transit", "delivered"].includes(order.status) && (
                <button
                  onClick={handleCashCollected}
                  disabled={cashLoading}
                  className="w-full py-2.5 sm:py-3 text-xs sm:text-sm bg-orange-500 hover:bg-orange-600 text-white font-medium rounded-lg flex items-center justify-center gap-2 transition-colors disabled:opacity-50"
                >
                  {cashLoading ? (
                    "Confirming..."
                  ) : (
            ...
            {isDelivered && (
              <div className="flex items-center justify-center gap-2 p-3.5 bg-green-50 dark:bg-green-950/30 rounded-xl border border-green-200 dark:border-green-800">
          ...
          {isActive && (
            <div
              className={`card p-3 flex items-center gap-2.5 ${gpsError ? "border-orange-200 bg-orange-50 dark:bg-orange-950/30" : ""}`}
            >
              {gpsError ? (
                <>
                  <AlertTriangle className="h-4 w-4 text-orange-500 flex-shrink-0" />
                ...
                <>
                  <div className="w-2 h-2 sm:w-2.5 sm:h-2.5 rounded-full bg-green-500 animate-pulse flex-shrink-0" />
                  <p className="text-[11px] sm:text-xs text-gray-600 dark:text-gray-400">
                    GPS active — sharing location
                  </p>
                  <span className="ml-auto text-[10px] sm:text-xs text-gray-400 tabular-nums truncate max-w-[120px] text-right">
                    {location.lat.toFixed(4)}, {location.lng.toFixed(4)}
                  </span>
                </>
              ) : (
          ...
          {isActive && (
            <div className="card p-4 sm:p-5">
              <h3 className="font-semibold text-xs sm:text-sm text-gray-800 dark:text-gray-300 mb-3 flex items-center gap-2">
                <MessageSquare className="h-4 w-4 text-gray-400 flex-shrink-0" />
                Chat with Customer
              </h3>
              <div className="h-40 sm:h-48 overflow-y-auto mb-3 space-y-2 pr-1">
                {messages.length === 0 ? (
                  <p className="text-xs text-gray-400 text-center pt-6">
                    No messages yet
                  </p>
                  ...
                  messages.map((m, i) => {
                    ...
                    return (
                      ...
                      >
                        <div
                          className={`max-w-[85%] sm:max-w-[80%] px-3 py-2 rounded-xl text-xs sm:text-sm ${
                            isMe
                              ? "bg-primary-600 text-white rounded-br-sm"
                        ...
                        >
                          {m.message}
                          <p
                            className={`text-[9px] sm:text-[10px] mt-1 ${isMe ? "text-primary-200" : "text-gray-400"}`}
                          >
                            {new Date(m.createdAt).toLocaleTimeString("en-IN", {
                              hour: "2-digit",
                              minute: "2-digit",
      ...
      {showOtpModal && (
        <>
          <div
            ...
            onClick={() => setShowOtpModal(false)}
          />
          <div className="card fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-50 bg-white dark:bg-gray-900 w-[90%] max-w-sm rounded-2xl shadow-2xl overflow-hidden">
            <div className="p-4 sm:p-5 border-b border-gray-100 dark:border-gray-800">
              <h3 className="font-semibold text-xs sm:text-sm text-gray-900 dark:text-gray-200 flex items-center gap-2">
                <KeyRound className="h-4 w-4 text-yellow-500 flex-shrink-0" /> Pickup OTP Verification
              </h3>
              ...
              {!otpSent ? (
                ...
                <>
                  <p className="text-xs text-green-600 dark:text-green-400 font-medium flex items-center gap-1">
                    <CheckCircle className="h-3.5 w-3.5 flex-shrink-0" /> OTP sent to customer
                  </p>
                  <div>
                    ...
                    <input
                      ...
                      placeholder="6-digit OTP"
                      className="input-field text-center text-base sm:text-lg tracking-widest font-mono"
                    />
                  </div>
                  <button
                    onClick={handleVerifyOtp}
                    disabled={otpLoading || otpInput.length !== 6}
                    className="btn-primary w-full text-xs sm:text-sm py-2.5 disabled:opacity-50"
                  >
                    {otpLoading ? "Verifying..." : "Verify & Confirm Pickup"}
                  </button>
                </>
      ...
      {showDelOtpModal && (
        <>
          ...
          <div className="card fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-50 bg-white dark:bg-gray-900 w-[90%] max-w-sm rounded-2xl shadow-2xl overflow-hidden">
            <div className="p-4 sm:p-5 border-b border-gray-100 dark:border-gray-800">
              <h3 className="font-semibold text-xs sm:text-sm text-gray-900 dark:text-gray-200 flex items-center gap-2">
                <KeyRound className="h-4 w-4 text-blue-500 flex-shrink-0" /> Delivery OTP Verification
              </h3>
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                Ask the receiver for the OTP sent to their phone
              </p>
            ...
            <div className="p-4 sm:p-5 space-y-4">
              {!deliveryOtpSent ? (
                ...
                <>
                  <p className="text-xs text-green-600 dark:text-green-400 font-medium flex items-center gap-1">
                    <CheckCircle className="h-3.5 w-3.5 flex-shrink-0" /> OTP sent to receiver
                  </p>
                  <div>
                    <label className="block text-xs sm:text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">
                      Enter Receiver OTP
      ...
      {showProofModal && (
        <>
          ...
          <div className="card fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-50 bg-white dark:bg-gray-900 w-[90%] max-w-sm rounded-2xl shadow-2xl overflow-hidden">
            <div className="p-4 sm:p-5 border-b border-gray-100 dark:border-gray-800">
              <h3 className="font-semibold text-xs sm:text-sm text-gray-900 dark:text-gray-200 flex items-center gap-2">
                <Camera className="h-4 w-4 text-primary-600 flex-shrink-0" /> Upload Delivery Proof
              </h3>
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                Take a photo of the delivered package at the drop location
              </p>
            ...
            <div className="p-4 sm:p-5 space-y-4">
              <label className="block cursor-pointer">
                ...
                {proofPreview ? (
                  <div className="relative">
                    ...
                    />
                    <p className="text-xs text-center text-primary-600 dark:text-primary-400 mt-2">
                      Tap to change photo
                    </p>
                  </div>
                ) : (
                  <div className="h-36 sm:h-40 border-2 border-dashed border-gray-200 dark:border-gray-700 rounded-xl flex flex-col items-center justify-center gap-2 hover:border-primary-400 transition-colors">
              ...
              <div className="flex gap-2 sm:gap-3">
                <button
                  onClick={closeProofModal}
                  className="flex-1 btn-secondary text-xs sm:text-sm py-2.5"
                >
                  Cancel
                </button>
                <button
                  onClick={handleProofSubmit}
                  disabled={!proofFile || proofLoading}
                  className="flex-1 btn-primary text-xs sm:text-sm py-2.5 disabled:opacity-50 flex items-center justify-center gap-2"
                >
                  {proofLoading ? (
                    <>
                      <span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />{" "}
                    ...
                    <>
                      <Upload className="h-4 w-4" /> Confirm
                    </>
                  )}
                </button>
              </div>
