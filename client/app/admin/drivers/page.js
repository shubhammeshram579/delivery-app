...
export default function AdminDriversPage() {
  ...
  const getDateStatusClass = (dateString) => {
    ...
    if (diffDays < 0) return 'text-red-600 font-semibold bg-red-50 px-1.5 py-0.5 rounded';
    if (diffDays <= 7) return 'text-amber-600 font-semibold bg-amber-50 px-1.5 py-0.5 rounded';
                ...
                <tr key={u.id} className="hover:bg-slate-50/50 transition">
                  ...
                  <td className="px-6 py-4">
                    {u.driverProfile?.isVerified ? (
                      ...
                      <span className="inline-flex items-center gap-1 text-xs font-semibold bg-red-50 text-red-700 px-2.5 py-1 rounded-md border border-red-200">⚠ Rejected</span>
                    ) : (
                      <span className="inline-flex items-center gap-1 text-xs font-semibold bg-amber-50 text-amber-700 px-2.5 py-1 rounded-md border border-amber-200">🕒 Pending</span>
                    )}
                  </td>
