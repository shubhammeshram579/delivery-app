...
//                         className={`text-xs px-2.5 py-1 font-medium rounded-lg transition-colors ${
//                           o.status === "pending"
...
//         {/* 3. Pass fetchOrders as a callback function to the modal */}
//         <AdminReassignModal
//           orderId={reassignOrderId}
//           open={!!reassignOrderId}
//           onClose={() => setReassignOrderId(null)}
//           onAssigned={fetchOrders} // Correctly triggers the update!
//         />
//       </div>
//     </DashboardLayout>
//   );
...
import { adminService } from "../../../services";
import { Package, Search, Filter, MoreVertical, Eye, RefreshCw, X, AlertTriangle, MapPin, CreditCard, ShieldCheck } from "lucide-react";
import { format } from "date-fns";
                ...
                orders.map((o) => (
                  <tr key={o.id} className="hover:bg-gray-50/80 transition-colors">
                    ...
                    <td className="px-4 py-3 relative">
                      <div className="inline-block" ref={activeDropdownId === o.id ? dropdownRef : null}>
                        <button
                        ...
                        {activeDropdownId === o.id && (
                          <div className="absolute right-4 mt-1 w-44 bg-white border border-gray-100 rounded-lg shadow-lg py-1 z-50">
                            <button
                              onClick={() => {
...
function AdminViewOrderModal({ order, onClose }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-xs flex justify-end z-[100] transition-opacity">
      <div className="w-full max-w-lg bg-white h-full flex flex-col shadow-2xl animate-slide-in overflow-y-auto">
        
        {/* Modal Header */}
        <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between sticky top-0 bg-white z-10">
          <div>
            <h2 className="text-lg font-bold text-gray-900">Order Deep-Dive</h2>
            <p className="text-xs font-mono text-gray-400">{order.orderNumber || order.id}</p>
          </div>
        ...
        <div className="p-6 space-y-6 flex-1">
          ...
          <div className="flex items-center justify-between bg-slate-50 p-3.5 rounded-xl border border-slate-100">
            <div>
              <span className="text-xs text-slate-400 block font-medium uppercase tracking-wider">Current Status</span>
              <StatusBadge status={order.status} />
            </div>
            <div className="text-right">
              <span className="text-xs text-slate-400 block font-medium">Type</span>
              <span className="text-sm font-semibold text-slate-700 capitalize">{order.orderType}</span>
            </div>
          </div>
          ...
          {order.wasDriverCancelled && (
            <div className="bg-amber-50 border border-amber-200 p-4 rounded-xl flex items-start gap-3">
              ...
              <div>
                <h4 className="text-sm font-bold text-amber-800">Driver Cancellation Recorded</h4>
                <p className="text-xs text-amber-700 mt-1">
                  A driver cancelled this order stating: <span className="font-semibold">"{order.driverCancelReason || "No reasons shared"}"</span>
                </p>
              </div>
            </div>
          )}
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="p-4 bg-gray-50/50 rounded-xl border border-gray-100">
              <span className="text-xs font-bold text-primary-700 uppercase tracking-wider">Customer (Sender)</span>
              <h3 className="text-sm font-bold text-gray-800 mt-1">{order.customer?.name}</h3>
              <p className="text-xs text-gray-500 mt-1">{order.customer?.phone}</p>
              <p className="text-xs text-gray-400 truncate mt-0.5">{order.customer?.email}</p>
            </div>
            
            <div className="p-4 bg-gray-50/50 rounded-xl border border-gray-100">
              <span className="text-xs font-bold text-teal-700 uppercase tracking-wider">Recipient (Receiver)</span>
              <h3 className="text-sm font-bold text-gray-800 mt-1">{order.receiverName}</h3>
              <p className="text-xs text-gray-500 mt-1">{order.receiverPhone}</p>
              {order.receiverAlternatePhone && (
                <p className="text-xs text-gray-400 mt-0.5">Alt: {order.receiverAlternatePhone}</p>
              )}
            </div>
          </div>
          ...
          <div className="space-y-3">
            <h4 className="text-xs font-bold text-gray-400 uppercase tracking-wide">Delivery Route Info</h4>
            <div className="space-y-4 relative pl-4 border-l-2 border-dashed border-gray-200">
              ...
              <div className="relative">
                ...
                <span className="text-xs font-bold text-gray-500">Pickup Address</span>
                <p className="text-xs text-gray-700 mt-0.5">{order.pickupAddress}</p>
              </div>
              {/* Drop-off Address */}
              <div className="relative">
                <MapPin className="h-4 w-4 text-red-500 absolute -left-[25px] bg-white ring-4 ring-white" />
                <span className="text-xs font-bold text-gray-500">Drop-off Address</span>
                <p className="text-xs text-gray-700 mt-0.5">{order.dropAddress}</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4 mt-3 pt-3 border-t border-gray-100">
              <div>
                ...
                <span className="text-sm font-bold text-gray-800">{order.distance} km</span>
              </div>
              <div>
                <span className="text-xs text-gray-400 block">Estimated Duration</span>
                <span className="text-sm font-bold text-gray-800">{order.estimatedTime} mins</span>
              </div>
            </div>
          </div>
          ...
          <div className="p-4 rounded-xl border border-gray-150">
            <h4 className="text-xs font-bold text-gray-400 uppercase tracking-wide mb-3">Assigned Driver</h4>
            {order.driver ? (
              <div className="space-y-2">
                <div className="flex justify-between text-sm">
                  <span className="font-bold text-gray-800">{order.driver.user?.name || "Driver details missing"}</span>
                ...
                <div className="grid grid-cols-2 gap-2 text-xs text-gray-500 pt-2 border-t border-gray-50">
                  ...
                  <span>Driver Status: <span className="capitalize font-medium">{order.driverStatus || "N/A"}</span></span>
                </div>
              </div>
            ) : (
              <p className="text-xs text-gray-400 italic">No driver is currently attached to this delivery.</p>
            )}
          </div>

          {/* Package Details */}
          <div className="p-4 bg-gray-50/50 rounded-xl border border-gray-100 space-y-3">
            <h4 className="text-xs font-bold text-gray-400 uppercase tracking-wide">Package Description</h4>
            <div className="grid grid-cols-2 gap-y-3 text-xs">
              <div>
                <span className="text-gray-400 block">Category</span>
                <span className="font-semibold text-gray-800 capitalize">{order.packageCategory}</span>
              </div>
              <div>
                <span className="text-gray-400 block">Fragile Cargo</span>
                <span className={`font-semibold ${order.isFragile ? "text-red-600" : "text-gray-800"}`}>
                  {order.isFragile ? "Yes ⚠️" : "No"}
                </span>
              </div>
              <div>
                <span className="text-gray-400 block">Declared Value</span>
                <span className="font-semibold text-gray-800">₹{order.packageValue}</span>
              </div>
              <div>
                <span className="text-gray-400 block">Declared Weight</span>
                <span className="font-semibold text-gray-800">{order.packageWeight} kg</span>
              </div>
            </div>
          ...
          <div className="p-4 border border-gray-150 rounded-xl space-y-4">
            ...
            <div className="grid grid-cols-2 gap-2 pt-2 border-t border-gray-100 text-xs">
              <div>
                ...
                <span className="font-semibold uppercase text-gray-700">{order.paymentMethod}</span>
              </div>
              <div>
                <span className="text-gray-400 block">Payment Status</span>
                <span className={`font-semibold capitalize ${order.payment?.status === "success" ? "text-green-600" : "text-amber-600"}`}>
            ...
            <div className="bg-emerald-50/50 p-3.5 rounded-lg border border-emerald-100 flex items-start gap-2.5">
              <ShieldCheck className="h-5 w-5 text-emerald-600 shrink-0 mt-0.5" />
              <div>
                <h5 className="text-xs font-bold text-emerald-800">Verification Integrity</h5>
                <p className="text-[11px] text-emerald-700 mt-0.5">
                  Delivery OTP Verification: <span className="font-bold">{order.deliveryOtpVerified ? "Verified" : "Unverified"}</span>
                </p>
                {order.cashCollectedAt && (
                  <p className="text-[11px] text-emerald-700">
                    Cash collected at: <span className="font-medium">{format(new Date(order.cashCollectedAt), "PPp")}</span>
                  </p>
                )}
              </div>
            </div>
            ...
            {order.deliveryProofImage && (
              <div className="pt-2">
                <span className="text-xs text-gray-400 block mb-2">Delivery Proof Photo:</span>
                <a 
